...
public class SimpleCycle {
    ...
    /**
     * Decides if the number is prime or not. DOES NOT WORK YET BECAUSE IT THINGS THE 2 IS NOT PRIME....
     * @param number input Integer for parameter.
     */
    private static void isPrime(Integer number) {
        int i = 2;
        double radix = Math.round(Math.sqrt(number));
        while (number % i != 0 && i <= radix) {
            i++;
        }
        if (i > number) {
